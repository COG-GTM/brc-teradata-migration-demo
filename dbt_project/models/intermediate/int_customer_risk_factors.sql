...
with customer_accounts as (
    ...
    select
        ...
        (date_part('year', current_date) - date_part('year', c.date_of_birth))::int as customer_age,

        -- Tenure in years
        (date_part('year', current_date) - date_part('year', c.onboarding_date))::int as tenure_years,
...
risk_factors as (
    ...
    select
        ...
        -- generate_surrogate_key returns text hash; convert first 8 hex chars to integer
        mod(
            abs(
                ('x' || substring({{ dbt_utils.generate_surrogate_key(['ca.customer_id']) }}, 1, 8))::bit(32)::int
            ),
            10
        ) as risk_bucket

...
with customer_accounts as (
    ...
    from {{ ref('int_unified_customer') }} c
    left join {{ ref('int_unified_account') }} a
...
transaction_metrics as (
    ...
    select
        ...
        count(case when t.value_band = 'HIGH_VALUE' then 1 end) as large_transaction_count

    from {{ ref('int_unified_account') }} a
    inner join {{ ref('int_unified_transaction') }} t
...
risk_factors as (
    ...
    select
        ...
        -- Logic drift resolution: adopted Teradata priority order (verified correct per Basel III)
        case
